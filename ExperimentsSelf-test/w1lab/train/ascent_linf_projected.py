...
import torch
from w1lab.layers import project_module_op_linf_
...
def train_dual_linf_projected(
    ...
    for step in range(1, steps + 1):
        ...
        for _ in range(proj_iters):
            project_module_op_linf_(critic, tau_linear=tau)

        if step % log_every == 0 or step == 1 or step == steps:
            est = _dual_estimate(critic, provider, eval_mc)
            gap = abs(est - true)
            elapsed = time.time() - t0
            print(f"[linf-proj] step {step:4d} | dual {est:.4f} | true {true:.4f} | gap {gap:.4f}")
