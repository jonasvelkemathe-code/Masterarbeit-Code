...
from w1lab.data import make_provider_cones2d
...
#Wir setzen hier die Parameter für die Algorithmen
STEPS = 400
BS = 512
LR = 1e-3
EVAL_MC = 10000
...
SEED = 123
OUTDIR = Path("runs/cone2d/single_theta")
...
provider = make_provider_cones2d(device)
...
for method in methods:
    if method == "bjorck":
        critic = make_critic(d=2, width=WIDTH, depth=DEPTH, method="bjorck", bjorck_iters=BJORCK_ITERS).to(device)
        ...
        critic = make_critic(d=2, width=WIDTH, depth=DEPTH, method="spectral").to(device)
        ...
        critic = make_critic(d=2, width=WIDTH, depth=DEPTH, method="parseval").to(device)
...
plt.title("2D-Kegelverteilung")
plt.grid(True)
plt.ylim(0, 1.2)
plt.legend()
plt.tight_layout()

out_png = OUTDIR / "vergleich_methoden_cone2d_single_theta.png"
